  ...
  Widget buildResults(BuildContext context) {
    return FutureBuilder<GeoJsonFeatureCollection>(
      future: client.geocodeSearchGet(
          text: query,
          boundaryCountry: 'TM',
          boundaryCircleCoordinate:
              const ORSCoordinate(latitude: 37.9500, longitude: 58.3833)),
                  ...
                  child: InkWell(
                    onTap: () {
                      close(
                        ...
                        SearchResult(
                          name: props.name,
                          box: BoundingBox.fromGeoPoints(List<GeoPoint>.from(
                              pr[index].bbox!.map((e) => GeoPoint(
                                  latitude: e.latitude,
                          ...
                          Column(
                              ...
                              children: [
                                ...
                                if (props.street != null)
                                  SizedBox(
                                      width: ini,
                                      child: Text(" Köçe: ${props.street} ")),
                                if (props.housenumber != null)
                                  SizedBox(
                                      width: ini,
                                      child: Text("Jaý: ${props.housenumber}"))
                              ]),
                          ...
                          SearchResult(
                              ...
                              box: pr[index].bbox == null
                                  ...
                                  : BoundingBox.fromGeoPoints(
                                      List<GeoPoint>.from(pr[index].bbox!.map(
                                          (e) => GeoPoint(
                                              latitude: e.latitude,
                                              longitude: e.longitude)))),
                              coordinates: pr[index].geometry.coordinates[0]
                                  [0]));
                    },
                    child: Container(
                      width: ini,
